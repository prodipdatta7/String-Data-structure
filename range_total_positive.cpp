...
void upd(int i , int b , int e , int l, int r , int k){
	if(lazy[i]){
		propagate(i , b , e) ;
	}
	...
	int mid = (b + e) >> 1 ;
	upd(i << 1 , b , mid , l , r , k) ;
	upd(i << 1 | 1, mid + 1 , e , l , r , k) ;
	tree[i] = max(tree[i << 1] , tree[i << 1 | 1]) ;
}

void query(int i , int b , int e , int l, int r){
	if(lazy[i]){
		propagate(i , b , e) ;
	}
	if(b > r or e < l or tree[i] <= 0)return ;
	if(b == e){
		bit.upd(b , 1) ;
		tree[i] = LLONG_MIN ;
		return ;
